...
- **Hands-on lab guides** — `hands-on-lab/` provides self-contained HTML lab guides: one per chapter (Ch1–Ch6), four capstone labs (main, B, C, D), and three reference pages (slash commands, plugins, directory structure), with an `index.html` portal page as the entry point.
...
- **핸즈온랩 가이드** — `hands-on-lab/`에 챕터별 랩 6개(Ch1–Ch6), 캡스톤 랩 4개(main, B, C, D), 참조 문서 3개(슬래시 커맨드, 플러그인, 디렉토리 구조)가 독립 실행형 HTML로 제공되며, 진입점 역할을 하는 `index.html` 포털 페이지가 포함됩니다.
